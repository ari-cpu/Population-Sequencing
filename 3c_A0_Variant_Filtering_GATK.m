...
%same script only slighty adjusted for GATK input 
clearvars
...
%mode="mono";
mode="pop";
path = "/home/isabel/shared_Master/Matlab/";    % Where are the unfiltered variant lists 
                                %(*_bcfcall.vcf) and where do you 
                                % want to save the SNPSummary?
%filepath = path+"pipeline_output_"+mode+"\"; % + "RAW\";
filepath ="/home/isabel/shared_Master/Matlab/bcfcall/mock/"

filesuffix = "_gatk.vcf";
...
elseif strcmp(sampleType, "customized")
    ...
    %sampleNames = ["Bsub_MockReads_change1" "Bsub_MockReads_change2" "Bsub_MockReads_change3" "Bsub_MockReads_change4"];
    sampleNames=["Bsub_MockReads_Mix_changeAll_RG_snp"];
...
minTotalCount = 0;     % What is your total count cut off ? (default: 50)
...
unambiFreq = 0;     % [in %]; When is a variant unambigious ? (default : 90)
...
for sampleCount = 1 : length(allSamples)
    ...
    for cycleBackCounter = 1 : length(cycles)
        ...
        for alts = 1:3
            ...
            %hier hier hier
            countArrs(maskPos) = num2cell(str2double(split(splittedCounts(maskPos,2),",",2)),2);
...
if saveSNPSummary
    ...
    elseif strcmp(sampleType, "customized")
            ...
            save(savepath + datestr(now, 'yyyymmdd') + "_gatk_SNPSummary.mat", 'SNPSummary')
